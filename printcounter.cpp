...
void PrintCounter::showStats(bool compact = false) {

  if (compact){
    SERIAL_ECHO_START;
    SERIAL_ECHOPGM("STATS");
    SERIAL_ECHOPAIR(":", this->data.totalPrints);
    SERIAL_ECHOPAIR("|", this->data.finishedPrints);
    SERIAL_ECHOPAIR("|", this->data.totalPrints - this->data.finishedPrints
      - ((this->isRunning() || this->isPaused()) ? 1 : 0));
    SERIAL_ECHOPAIR(":", this->data.printTime);
    SERIAL_ECHOPAIR("|", this->data.longestPrint);
    SERIAL_ECHOPAIR(":", this->data.filamentUsed);
  } else {
    char buffer[21];
    duration_t elapsed;
