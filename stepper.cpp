...
*/
  ...
  #define Y_APPLY_DIR(v,Q) do{ Y_DIR_WRITE(v); Y2_DIR_WRITE((v) != INVERT_Y2_VS_Y_DIR); }while(0)
    ...
    if (performing_homing) { \
      if (Y_HOME_DIR < 0) { \
        if (!(TEST(endstops.old_endstop_bits, Y_MIN) && (count_direction[Y_AXIS] < 0))) Y_STEP_WRITE(v); \
        if (!(TEST(endstops.old_endstop_bits, Y2_MIN) && (count_direction[Y_AXIS] < 0))) Y2_STEP_WRITE(v); \
      } \
      else { \
        if (!(TEST(endstops.old_endstop_bits, Y_MAX) && (count_direction[Y_AXIS] > 0))) Y_STEP_WRITE(v); \
        if (!(TEST(endstops.old_endstop_bits, Y2_MAX) && (count_direction[Y_AXIS] > 0))) Y2_STEP_WRITE(v); \
      } \
    } \
    else { \
      Y_STEP_WRITE(v); \
