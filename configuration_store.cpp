...
void Config_Postprocess() {
  ...
  #ifdef PREV_EEPROM_VERSION
    /**
     * Retrieve Configuration settings from previous printer versions
     */
    int Config_RetrieveOldSettings(char *stored_ver) {
      int retrieved_values = 0; 
      const char prev_version[4] = PREV_EEPROM_VERSION;
      if (strncmp(prev_version, stored_ver, 3) == 0) {
        
        #ifdef PREV_EEPROM_ZPO_INDEX
          float stored_zprobe_zoffset;
          int index = PREV_EEPROM_ZPO_INDEX;
          _EEPROM_readData(index, (uint8_t*)&stored_zprobe_zoffset, sizeof(stored_zprobe_zoffset));
          zprobe_zoffset = -stored_zprobe_zoffset;
          retrieved_values++;
          SERIAL_ECHO_START;
          SERIAL_ECHOPAIR("Retrieved Z_PROBE_OFFSET from previous version: ", zprobe_zoffset);
        #endif
        
        #if ENABLED(PRINTCOUNTER)
          #if defined(PREV_EEPROM_LIFETIME_MAGIC) && defined(PREV_EEPROM_LIFETIME_OFFSET) && defined(CURR_EEPROM_STATS_OFFSET)
            unsigned long magic = eeprom_read_dword((uint32_t*)(PREV_EEPROM_LIFETIME_OFFSET + 0));
            if (magic == PREV_EEPROM_LIFETIME_MAGIC){
              unsigned long lifetime_print_minutes = eeprom_read_dword((uint32_t*)(PREV_EEPROM_LIFETIME_OFFSET + 8)); // to sec (uint32_t)
              unsigned long lifetime_print_centimeters = eeprom_read_dword((uint32_t*)(PREV_EEPROM_LIFETIME_OFFSET + 12)); // to mm (double)
              printStatistics stats = { 0, 0, (uint32_t)(lifetime_print_minutes * 60UL), 0, (double)(lifetime_print_centimeters * 10UL) };
              eeprom_update_block(&stats,
                (void *)(CURR_EEPROM_STATS_OFFSET + sizeof(uint8_t)), sizeof(printStatistics));
              eeprom_write_byte((uint8_t *)(CURR_EEPROM_STATS_OFFSET + 0), 0x16);
  ...
  void Config_RetrieveSettings() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      Config_ResetDefault();
      if (Config_RetrieveOldSettings(stored_ver) > 0){
        force_save = true;
      }
    }
    else {
