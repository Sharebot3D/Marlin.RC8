...
/**
 * Cartesian Resume Position
 *   Used by pause / resume commands (M990 / M991) to set / retrieve original position before pause.
 */
static float resumepos[XYZE] = { 0.0 };
...
// External click
  ...
  void setup_extclickpin() {
    SET_INPUT(EXT_CLICK_PIN);
    #if ENABLED(ENDSTOPPULLUP_EXT_CLICK)
...
void sendM613Status( bool ok ){
           ...
           SERIAL_ECHO_START;
           SERIAL_ECHOPGM("M613");
           SERIAL_ECHO(lcd_getstatus(&level)); //TODO: verificare get_status()
           SERIAL_ECHO(";");
           SERIAL_ECHO(level);
#if EXTRUDERS > 0
           SERIAL_ECHOPGM(";0:");
           SERIAL_ECHO(int(thermalManager.degHotend(0) + 0.5 ));
           SERIAL_ECHOPGM("/");
           SERIAL_ECHO(int(thermalManager.degTargetHotend(0) + 0.5 ));
#endif
#if EXTRUDERS > 1
           SERIAL_ECHOPGM(";1:");
           SERIAL_ECHO(int(thermalManager.degHotend(1) + 0.5 ));
           SERIAL_ECHOPGM("/");
           SERIAL_ECHO(int(thermalManager.degTargetHotend(1) + 0.5 ));
#endif
#if TEMP_SENSOR_BED != 0
           SERIAL_ECHOPGM(";B:");
           SERIAL_ECHO(int(thermalManager.degBed() + 0.5 ));
           SERIAL_ECHOPGM("/");
           SERIAL_ECHO(int(thermalManager.degTargetBed() + 0.5 ));
#endif
           ...
           SERIAL_ECHOPGM(";");
           SERIAL_ECHOLN(itostr3(feedrate_percentage));
           if ( ok ) {
             SERIAL_ECHOLNPGM("ok");
           }
    ...
    if (axis == Z_AXIS) {
        ...
        current_position[Z_AXIS] -= (zprobe_zoffset + zprobe_zoffset_delta);
          ...
          if (DEBUGGING(LEVELING)) {
            ...
            SERIAL_ECHOLNPAIR("> zprobe_zoffset = ", zprobe_zoffset + zprobe_zoffset_delta);
          }
        #endif
...
static void clean_up_after_endstop_or_probe_move() {
  ...
  inline void do_probe_raise(float z_raise) {
    ...
    if ((zprobe_zoffset + zprobe_zoffset_delta) < 0) z_dest -= (zprobe_zoffset + zprobe_zoffset_delta);
 ...
 // TRIGGERED_WHEN_STOWED_TEST can easily be extended to servo probes, ... if needed.
  ...
  static float run_z_probe() {
    ...
    refresh_cmd_timeout();
      ...
      if ((zprobe_zoffset + zprobe_zoffset_delta) < 0) z -= (zprobe_zoffset + zprobe_zoffset_delta);
...
static void homeaxis(AxisEnum axis) {
  ...
  // Set a flag for Z motor locking
    ...
    if (axis == Y_AXIS) stepper.set_homing_flag(true); //TODO: aggiungere asse per differenziare ?
  #endif
  ...
  inline void gcode_G29() {
    ...
    float xProbe = 0, yProbe = 0, measured_z = 0;
      ...
      const float xGridSpacing = (right_probe_bed_position - left_probe_bed_position) / (abl_grid_points_x - 1),
        ...
        float zoffset = zprobe_zoffset + zprobe_zoffset_delta;
    ...
    // Calculate leveling, print reports, correct the position
      ...
      if (!dryrun) {
        ...
        if ( NEAR(current_position[X_AXIS], xProbe - (X_PROBE_OFFSET_FROM_EXTRUDER))
          ...
          float simple_z = z_real - (measured_z - (-(zprobe_zoffset + zprobe_zoffset_delta)));
          #if ENABLED(DEBUG_LEVELING_FEATURE)
            if (DEBUGGING(LEVELING)) {
...
*/
    if (code_seen('Q'))
      print_job_timer.showStats( true );
    else
...
inline void gcode_M503() {
  ...
  inline void gcode_M851() {
    ...
    float value;
    if (code_seen('Z')) {
      value = code_value_axis_units(Z_AXIS);
  ...
  inline void gcode_M600() {
    ...
    millis_t t_exit = millis() + 1000UL;
    while (LCD_CLICKED || PENDING(millis(), t_exit)){
      idle( true );
    }
    ...
    #if ENABLED(FILAMENT_CHANGE_SINGLE_EXTRUSION)

      // Prevent unlimited extrusion in case of error
      unsigned long extrusion_timeout = (unsigned long) FILAMENT_CHANGE_EXTRUSION_TIMEOUT * 1000UL;
      if(code_seen('S'))
      {
          extrusion_timeout = (unsigned long) code_value_int() * 1000UL;
      }
      millis_t now, extrusion_stop_time;
      extrusion_stop_time = millis() + extrusion_timeout;
      bool extrude = true;
    ...
    #else // !FILAMENT_CHANGE_SINGLE_EXTRUSION
    ...
    #endif // FILAMENT_CHANGE_SINGLE_EXTRUSION
    ...
    #if ENABLED(USE_EXTERNAL_CLICK)
      LCD_MESSAGEPGM(MSG_FILAMENT_CHANGE_RES);
    #endif
...
inline void gcode_M71() {
  // Invokes M0/M1 without parenthesis
  if(code_seen(')')) {
     *(seen_pointer)='\0';
  }
  if(code_seen('(')) {
    current_command_args = seen_pointer + 1;
  }
  gcode_M0_M1();
}
...
inline void gcode_M990() {
  ...
  // Save current position of all axes
  LOOP_XYZE(i)
    resumepos[i] = destination[i] = current_position[i];

  // Initial retract before move to parking position
  if (code_seen('E')) destination[E_AXIS] += code_value_axis_units(E_AXIS);
  #if defined(PAUSERESUME_RETRACT_LENGTH) && PAUSERESUME_RETRACT_LENGTH > 0
    else destination[E_AXIS] -= PAUSERESUME_RETRACT_LENGTH;
  #endif

  line_to_destination(PAUSERESUME_RETRACT_FEEDRATE);

  // Lift Z axis
  float z_lift = code_seen('Z') ? code_value_axis_units(Z_AXIS) :
    #if defined(PAUSERESUME_Z_ADD) && PAUSERESUME_Z_ADD > 0
      PAUSERESUME_Z_ADD
    #else
      0
    #endif
...
inline void gcode_M991() {
  ...
  // Move XY to starting position, then Z, then unretract filament
  destination[X_AXIS] = resumepos[X_AXIS];
  destination[Y_AXIS] = resumepos[Y_AXIS];
  line_to_destination(PAUSERESUME_XY_FEEDRATE);
  destination[Z_AXIS] = resumepos[Z_AXIS];
  line_to_destination(PAUSERESUME_Z_FEEDRATE);
  destination[E_AXIS] = resumepos[E_AXIS];
  line_to_destination(PAUSERESUME_UNRETRACT_FEEDRATE);
...
#if ENABLED(FILAMENT_DETECTION_COUNTER)
  /**
   * M993: Set Filament detection ON / OFF
   *
   *   S0 Filament detection OFF
   *   S1 Filament detection ON
   *   
   *   P[factor] Multiply COUNTER LIMIT by factor
   *
   */
  inline void gcode_M993() {
    if (code_seen('S')){
      detect_filament = (code_value_byte() > 0);
    }
    if (code_seen('P')){
      float value = code_value_float();
    ...
    SERIAL_ECHO(detect_filament_factor);
    SERIAL_EOL;
  }
#endif
...
void process_next_command() {
  ...
  switch (command_code) {
    ...
    case 'M': switch (codenum) {
      ...
      case 71: // M71: Echo message and waits for user to click
        gcode_M71();
        break;

      case 990: // M990: Save current position and move to "parking" coordinates (away from object)
        gcode_M990();
        break;

      case 991: // M990: Move to saved coordinates
        gcode_M991();
        break;

      #if ENABLED(FILAMENT_DETECTION_COUNTER)
        case 993: // M993: Set Filament Detection ON / OFF (with correction factor)
          gcode_M993();
          break;
      #endif // FILAMENT_DETECTION_COUNTER
    }
    break;
...
void prepare_move_to_destination() {
  ...
  void handle_filament_runout() {
    if (!filament_ran_out
      #if ENABLED(FILAMENT_DETECTION_COUNTER)
        && (fd_count > (FILAMENT_DETECTION_LIMIT * detect_filament_factor))
      #endif
    ) {
